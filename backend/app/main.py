...
async def lifespan(app: FastAPI):
    ...
    # Public portfolio deployments are read-only.
    # Do not start Zoom synchronization or background workers.
    if not settings.public_demo_mode:
        ...
        if settings.sync_scheduler_enabled:
            ...
            scheduler.start(
                poll_interval=settings.sync_scheduler_poll_interval_seconds
            )
...
def create_app() -> FastAPI:
    ...
    allow_origins=[
        ...
        "https://zoom-agentic-ai-v2.vercel.app",
    ],
